...
def engineer_features_97(df):
    """
    使用已经计算好的97个JQ因子。
    输入df应包含原始列: 股票代码, 日期, 开盘, 收盘, 最高, 最低, 成交量, 成交额, 振幅, 涨跌额, 换手率, 涨跌幅
    以及97个JQ因子列。
    """
    df = df.copy()

    # 检查JQ因子是否存在于df中
    existing_factors = [col for col in JQ_FACTORS if col in df.columns]

    if len(existing_factors) < len(JQ_FACTORS):
        missing = set(JQ_FACTORS) - set(existing_factors)
        print(f"警告: 以下JQ因子不存在于数据中: {missing}")
...
def engineer_features_39plus97(df):
    """
    计算39个技术指标特征和97个JQ因子，并合并它们。
    输入df应包含原始列和97个JQ因子列。
    """
    df_copy = df.copy()
    ...
    # 97个JQ因子列
    feature_cols_97_exist = [col for col in JQ_FACTORS if col in df.columns]

    # 合并 - 保留df_39的所有列(包含原始列),只添加97因子
    df_final = pd.concat([df_39, df[feature_cols_97_exist]], axis=1)
...
def engineer_features_158plus97(df):
    """
    计算158个Alpha特征和97个JQ因子，并合并它们。
    输入df应包含原始列和97个JQ因子列。
    """
    df_copy = df.copy()

    # 1. 计算158个Alpha特征 - 保留所有原始列
    df_158 = engineer_features(df_copy)

    # 97个JQ因子列
    feature_cols_97_exist = [col for col in JQ_FACTORS if col in df.columns]

    # 合并 - df_158已包含所有原始列,只添加97因子
    df_final = pd.concat([df_158, df[feature_cols_97_exist]], axis=1)
...
def engineer_features_158plus39plus97(df):
    """
    计算158个Alpha特征 + 39个技术指标特征 + 97个JQ因子，并合并它们。
    输入df应包含原始列和97个JQ因子列。
    """
    df_copy = df.copy()
    ...
    # 合并
    df_final = pd.concat(
        [df_158, df_39[feature_cols_39_exist], df[feature_cols_97_exist]], axis=1
    )
...
def process_single_stock(stock_row, data, features, sequence_length, date):
    ...
    stock_history = (
        data[(data["instrument"] == stock_code) & (data["datetime"] <= date)]
...
def create_ranking_dataset_multiprocess(
    data, features, sequence_length, ranking_data_path=None, max_workers=None
):
    """
    ...
    try:
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            # 收集结果
            for future in tqdm(
                futures, desc="Processing dates", total=len(valid_dates)
            ):
                try:
    ...
    if ranking_data_path:
        joblib.dump(
            (sequences, targets, relevance_scores, stock_indices), ranking_data_path
        )
...
def create_dataset(data, features, sequence_length, ranking_data_path=None):
    ...
    return create_ranking_dataset_multiprocess(
        data, features, sequence_length, ranking_data_path
    )
...
def create_ranking_dataset_vectorized(
    ...
    for stock_code, group in tqdm(grouped, desc="Processing stocks"):
        ...
        for i in range(num_windows):
            ...
            # 滑动验证集不需要自然日连续，只要样本时间连续即可
            if require_natural_day_consecutive:
            ...
            # 存储窗口结束日期和未来5天日期范围
            future_dates = tuple(str(d)[:10] for d in dates[end_idx + 1 : end_idx + 6])
            all_windows.append((end_date, stock_code, seq, target, future_dates))

    # 4. 转为 DataFrame 便于按日期聚合
    print("Step 2: 按日期聚合窗口...")
    window_df = pd.DataFrame(
        all_windows, columns=["date", "stock_code", "seq", "target", "future_dates"]
    )
    ...
    # 统计每天的股票数量
    if verbose:
        date_counts = grouped_by_date.size()
        print(
            f"DEBUG: 每天股票数 - min: {date_counts.min()}, max: {date_counts.max()}, <10的日期数: {(date_counts < 10).sum()}"
        )
    ...
    for date, group in tqdm(grouped_by_date, desc="Aggregating by date"):
        if (
            min_window_end_date is not None
            and pd.to_datetime(date) < min_window_end_date
        ):
            continue
        ...
        # 打印每个样本的日期跨度
        future_dates = group["future_dates"].iloc[0]
        sample_info.append((str(date)[:10], future_dates))
    ...
    if verbose:
        ...
        for end_date, future in sample_info:
            print(f"  {end_date} -> {', '.join(future)}")
    print(f"成功创建 {len(sequences)} 个训练样本")
    if len(sequences) > 0:
