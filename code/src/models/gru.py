...
class RankingGRU(nn.Module):
    def __init__(self, input_dim, config, num_stocks, emb_dim=16):
        super(RankingGRU, self).__init__()
        self.model_type = "RankingGRU"
        self.config = config
    ...
    def forward(self, src):
        ...
        gru_out, h_n = self.gru(src_proj)

        attended = self.feature_attention(gru_out)
